...
echo '*/5 * * * * root /etc/scripts/mongodump.sh' >> /etc/crontab
#echo '5 0 * * * /etc/scripts/mongologcleanup.sh'>/etc/scripts/mongologcleanup.txt
...
sed -i:bak 's/mmsGroupId=/mmsGroupId=/' /etc/mongodb-mms/automation-agent.config
sed -i:bak 's/mmsApiKey=/mmsApiKey=/' /etc/mongodb-mms/automation-agent.config
